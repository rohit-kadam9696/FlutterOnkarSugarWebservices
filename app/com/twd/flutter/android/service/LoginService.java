...
LoginDao login=new LoginDao();
	...
	public LoginResponse appLogin(JSONObject reqObj,String imei,String accessType, LoginResponse res) {
		try(Connection conn=DBConnection.getConnection())
		{
			...
			if(res.isSuccess())
			{
				...
				if(res.isSuccess())
				{
					res=login.updateImeiAndRandamString(accessType,res,imei,mobileno,conn);
				}else
				{
					res=login.saveimeiAndRandamString(accessType,res,imei,mobileno,conn);
				}
				if(res.isSuccess())
				{
	...
	@Override
	public MainResponse checkAppUpdate(String chit_boy_id,String versionId,String accessType,MainResponse appversionRes) {
		try(Connection conn=DBConnection.getConnection())
		{
			appversionRes=login.checkAppUpdate(chit_boy_id,appversionRes,versionId,accessType,conn);
		}catch (Exception e) {
